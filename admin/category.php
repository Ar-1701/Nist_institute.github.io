...
if (mysqli_num_rows($result) > 0) {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        $table .= "<tr>
                            ...
                            <td>{$row["post"]}</td>
                            <td class='edit'><a href='update-category.php?id={$row['category_id']}' ><i class='bi bi-pencil-square'></i></a></td>
                            <td class='delete'><a href='delete-category.php?id={$row['category_id']}'><i class='bi bi-trash text-danger'></i></a></td>
                        </tr>";
        $serial++;
    }
    $table .= '</tbody></table>';
