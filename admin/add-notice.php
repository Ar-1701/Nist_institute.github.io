...
if (mysqli_num_rows($result) > 0) {
    while ($row = mysqli_fetch_assoc($result)) {
        echo "<option value='{$row['category_id']}' class='get_value'> {$row['category_name']} </option>";
    }
}
?>
                          </select>
                      </div>
                      <input type="submit" name="notice_submit" id="notice_submit" class="btn btn-primary" value="Save" required />
                  </form>
                  <!--/Form -->
              </div>
          </div>
